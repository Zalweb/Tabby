...
| **2026-09-14** | CEO / Security | Configure new Google Client ID and browser OAuth fallback. | Switched client ID to `137141086000-qnr5kedgh9miig5mmaaq90efn90gckmm.apps.googleusercontent.com` across `.env`, `supabase_tabby_repository.dart`, `Info.plist`, `AndroidManifest.xml`, and GitHub secrets. Implemented graceful browser OAuth fallback to prevent failure on sideloaded or unverified bundle signatures. Recompiled both `Tabby.apk` and `Tabby.ipa`. | `.env`, `supabase_tabby_repository.dart`, `ios/Runner/Info.plist`, `android/app/src/main/AndroidManifest.xml`, `releases/`. |
