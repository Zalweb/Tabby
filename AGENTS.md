...
| **2026-09-19** | Lead Mobile Architect & UI Engineer | Build native Flutter physics animation engine for Tabby mascot across all 9 emotional states with touch physics and particle overlays. | Upgraded `TabbyMascotWidget` into a full stateful animation engine: (1) Continuous sinusoidal breathing and floating loop at 60/120 fps with variable tempos and poses per emotion (`CELEBRATING` joyful hops, `SLEEPING` slow restful breathing and tilt, `CALCULATING` focused pulse, `USER_OWES` / `OVERDUE` anxious wobble). (2) Interactive touch squash-and-stretch on tap down and elastic release. (3) Spring emotion transitions (`Curves.elasticOut`) on avatar and rotating badge. (4) Ambient particle overlays: multi-colored confetti sparkles for celebration and floating 'z z z' sleep indicators. (5) `AnimatedSwitcher` fade-and-slide speech bubble transitions. (6) Guarded continuous loops with test environment detection to prevent test runner timeouts. Verified with dedicated test suite (`test/features/tabby_mascot_animation_test.dart`), 171/171 passing tests, and 0 analyzer issues. | `tabby_mascot_widget.dart`, `test/features/tabby_mascot_animation_test.dart`, `AGENTS.md`. |
